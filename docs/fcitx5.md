...
carries it there too (via its own `.desktop` override). Ozone/Wayland itself comes
from `ELECTRON_OZONE_PLATFORM_HINT=auto` in
[`environment.d/electron.conf`](../home/dot_config/environment.d/electron.conf) —
separate from IME. See [vscode.md](vscode.md) and [xdg-environment.md](xdg-environment.md).
